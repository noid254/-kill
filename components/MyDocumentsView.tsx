...
const MyDocumentsView: React.FC<MyDocumentsViewProps> = ({ documents, allDocuments, onScan, onSelectDocument, currentUser }) => {
    const [filter, setFilter] = useState<'Sent' | 'Received' | 'Assets'>('Sent');
    ...
    const filteredDocuments = useMemo(() => {
        ...
        switch (filter) {
            case 'Sent':
                return sorted.filter(doc => !doc.isAsset && doc.issuerName === currentUser?.name);
            case 'Received':
                return sorted.filter(doc => !doc.isAsset && doc.issuerName !== currentUser?.name);
            case 'Assets':
    ...
    const filters: ('Sent' | 'Received' | 'Assets')[] = ['Sent', 'Received', 'Assets'];
    ...
    return (
        <div className="p-4 bg-gray-50 min-h-full space-y-4">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
                <h2 className="font-semibold">Asset Vault: Ownership Check</h2>
