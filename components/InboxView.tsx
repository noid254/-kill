...
interface InboxViewProps {
    ...
    onAction: (type: 'saccoJoinRequest' | 'assetTransfer' | 'gigApplication', payload: any) => void;
...
const InboxMessageRow: React.FC<{ 
    ...
    onAction: (type: 'saccoJoinRequest' | 'assetTransfer' | 'gigApplication', payload: any) => void,
    onViewProviderProfile: (provider: ServiceProvider) => void,
}> = ({ message, onSelect, isSelected, allProviders, currentUserPhone, onAction, onViewProviderProfile }) => {
    ...
    const actionData = useMemo(() => {
        ...
        if (message.action.type === 'gigApplication') {
            return { type: 'gig', requesterId: message.action.requesterId, gigId: message.action.gigId };
        }
        return null;
    ...
    const renderActionUI = () => {
        ...
        if (actionData.type === 'gig') {
            const applicant = allProviders.find(p => p.id === actionData.requesterId);
            if (!applicant) return null;
            return (
                 <div className="space-y-2 mt-3">
                    <div className="flex gap-2">
                        <button onClick={(e) => { e.stopPropagation(); onViewProviderProfile(applicant); }} className="flex-1 bg-gray-200 text-gray-800 font-bold py-2 px-3 rounded-lg text-sm">View Profile</button>
                        {!showContact && <button onClick={(e) => { e.stopPropagation(); setShowContact(true); }} className="flex-1 bg-blue-500 text-white font-bold py-2 px-3 rounded-lg text-sm">Hire</button>}
                    </div>
                    {showContact && (
                        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                            <h4 className="font-bold text-blue-800">Contact Details</h4>
                            <p><strong>Phone:</strong> <a href={`tel:${applicant.phone}`} className="text-blue-600 hover:underline">{applicant.phone}</a></p>
                            {applicant.whatsapp && <p><strong>WhatsApp:</strong> <a href={`https://wa.me/${applicant.whatsapp}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{applicant.whatsapp}</a></p>}
                        </div>
                    )}
                </div>
            )
                ...
                <div>
                    {userMessages.map(msg => (
                        <InboxMessageRow 
                            ...
                            onAction={onAction}
                            onViewProviderProfile={onViewProviderProfile}
                        />
                    ))}
                </div>
            ) : (
