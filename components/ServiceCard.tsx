...
const ServiceCard: React.FC<{ provider: ServiceProvider; onClick: () => void }> = ({ provider, onClick }) => {
    return (
        <div onClick={onClick} className="bg-white rounded-lg shadow-sm overflow-hidden cursor-pointer w-full hover:shadow-lg transition-shadow duration-300 group">
            <div className="relative h-40">
                <img className="w-full h-full object-cover" src={provider.coverImageUrl} alt={provider.service} />
                {provider.isVerified && (
                    <div className="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1 shadow-md">
                        <VerifiedIcon className="w-3 h-3"/>
                    </div>
                )}
                <img src={provider.avatarUrl} alt={provider.name} className="absolute -bottom-5 left-3 w-12 h-12 rounded-full border-2 border-white shadow-md" />
                <div className="absolute bottom-2 right-2 bg-black bg-opacity-60 text-white text-[10px] px-2 py-1 rounded-full font-semibold flex items-center gap-1">
            ...
            <div className="p-3 pt-6 space-y-1">
                <h3 className="font-bold text-base text-brand-navy truncate">{provider.service}</h3>
                <div className="flex justify-between items-center">
                    <p className="text-sm text-gray-600 truncate">{provider.name}</p>
                ...
                <p className="text-lg font-bold text-brand-gold pt-1">
                    {provider.currency}
                    {provider.hourlyRate.toLocaleString()}
                    <span className="text-sm font-medium text-gray-500">/{rateSuffix[provider.rateType]}</span>
                </p>
            </div>
        </div>
    );
